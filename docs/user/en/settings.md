...
In Carrot Web, use **Settings > Display > External AI** to configure `External Phone AI` (`ExternalAIEnabled`), `External AI Video Transport` (`ExternalAITransport`), and `Show External AI Objects` (`ExternalAIShowOverlay`). External AI defaults to off (`0`), video transport to H.264 (`1`), and object display to on (`1`). `ExternalAIEnabled` enables the experimental path that sends road video to an external Android phone and receives its object detections. It is currently limited to visualization and communication diagnostics. Its data is not delivered to steering, acceleration, braking, radar, or the safety model, and it does not modify or online-train the C3/C3X/C4 `modeld` result. When enabled, `phoneaid` runs only while on-road and opens the TCP video port (default `7724`) and UDP result port (default `7725`). A disconnect or two seconds without a result immediately hides stale objects while normal driving operation continues.

Leave `ExternalAIPhoneIP` empty to accept any IPv4 client and result sender, or set it to accept only that address. `ExternalAIFramePort` selects the TCP video port and `ExternalAIResultPort` selects the UDP result port. The default H.264 mode uses the C3/C3X/C4 Qualcomm hardware encoder to send road video at 854×480, 20 FPS, and about 750 kbps, then decodes it with Android `MediaCodec`. With YouTube Live low-quality road mode, External AI shares the same 854×480 H.264 output instead of running a duplicate encoder. Medium, high, and wide YouTube modes use a different resolution or camera, so External AI uses its JPEG compatibility path. If the encoder is unavailable for three seconds, the same connection automatically falls back to 640×360 JPEG and returns after the next H.264 keyframe. If the H.264 network queue backs up, it discards the broken GOP and resumes only at the next IDR. `ExternalAIFrameFPS` (1–15, default 5) and `ExternalAIJpegQuality` (30–95, default 75) apply only to JPEG compatibility mode, where a pending old image is replaced by the newest one. The app's inference target is independently selectable from 1–20 FPS. `ExternalAIMaxLatencyMs` sets the maximum round-trip age using the monotonic timestamp originally attached by the CarrotPilot device (default 300 ms). End the current drive and start again after changing the transport. The connection is not encrypted, so use it only on a trusted, dedicated vehicle network and configure the phone IP.

With `ExternalAIShowOverlay` enabled, supported classes appear as class-specific 3D shapes on the external Cluster HUD. An external monitor is not required: the C3X camera view shows class-specific 2D silhouettes, object names following the current `LanguageSetting`, and confidence. Both the C3X view and external Cluster HUD provide object labels for English, German, French, Portuguese, Spanish, Turkish, Ukrainian, Thai, Traditional Chinese, Simplified Chinese, Korean, and Japanese; changing the UI language updates them from the same translation catalogs. A compact device status panel also shows the phone connection, inference backend, selected input size, `total latency/AI processing time`, and current object count. The device shows a green `eNPU` badge when the phone selects an NNAPI or QNN accelerated backend, or a blue `eCPU` badge for ONNX Runtime CPU or CPU fallback. Both badges remain hidden while disconnected or invalid, and the active compute badge remains visible when object overlays are disabled. `QNN` confirms one successful warm-up inference on a full-graph HTP session with CPU fallback disabled. Because NNAPI may distribute operations across an NPU, DSP, and GPU, its `eNPU` only means that the external accelerated path was requested, not that every operation was measured on a physical NPU. Traffic lights and stop signs currently use 2D symbols on the device only. Distance derived from bounding-box height is approximate until camera calibration is completed; when a detection matches a nearby front-radar object, only its class information is added to the radar position. On-device coordinates are also debug-grade approximations until the transmitted frame crop and calibration are finalized. `ExternalAIDebug` remains inactive until a later stage. Validate with static or recorded imagery and a mock sender before enabling it in a vehicle.

To test video transmission, result reception, and both display projections without Android or the monitor, run `python tools/external_ai/mock_phone_ai.py --host 127.0.0.1` on the C3X while the on-road view is active. It receives C3X frames without displaying them and returns dummy detections. `ExternalAIEnabled=1` and `ExternalAIShowOverlay=1` are required; stop it with `Ctrl-C`, or add `--count 1` for one round trip. To test only the previous result receiver without camera video, add `--results-only --fps 5`.

#### Connecting after installing the Android app

The Kotlin app for real-phone testing is under `tools/android_ai_client/`. It requires a 64-bit ARM phone with Android 10 or newer, and no YOLO ONNX model is bundled. For the first test, use dynamic-input YOLO11n Detection FP32 ONNX at the default 320 input. Use 416 for balance and 640 only to compare small-object quality; a static model uses its own input. Embedded NMS, segmentation, pose, classification, OBB, and end-to-end outputs are not supported. Press **권장 모델 다운로드** in the Android app and review the Ultralytics license notice to download the official v8.4.0 `yolo11n.onnx` directly. The app installs it internally only after the pinned size, SHA-256, and ONNX input/output checks all pass. Use **다른 ONNX 파일 선택** when offline or testing another compatible model. The source has no device-model gate, but C3X is the current validation target; C3 and C4 still require an on-device road-stream and UI test. See the [English Android installation and pairing guide](../../../tools/android_ai_client/README.en.md) for model provenance, license, and APK installation.

When a phone hotspot changes the C3X address, the app checks its saved address and then probes only the frame port in the local private IPv4 `/24`. It accepts and saves only a server whose first four bytes are JPEG `CAI1` or H.264 `CAI2`. With automatic connection enabled and a saved model, opening the app starts the discovery service and YOLO starts as soon as the device is found. Discovery is bounded to at most two local `/24` networks and does not scan the internet or an arbitrary port range. It does not start at boot, so reopen the app after a phone reboot or force-stop.
...
6. Press **권장 모델 다운로드** or select another compatible ONNX model, and initially leave input size at 320. Once a verified model is ready and automatic connection remains enabled, discovery starts without another button press. If discovery fails, enter the current device address and use **입력 IP로 시작**. Confirm **연결됨** plus an `eNPU` or `eCPU` badge.

The default v0.6.0 app selects a full-graph Qualcomm QNN/HTP session first, then NNAPI, then CPU. It binds the downloaded dynamic model to the selected 320, 416, or 640 input while opening QNN. If an operation is unsupported or HTP is unavailable, the status reports the QNN fallback reason and the app automatically tries NNAPI. NNAPI does not force NCHW; if that accelerated session also fails, the app falls back to CPU. Its status separates the rolling 120-sample phone average and p95 from JPEG decode, preprocessing, ORT, and postprocessing time. Keep 320 and a low FPS when p95 keeps rising above the average or Android reports a performance-limited thermal state. During discovery it does not open the model or NPU session and holds neither a wake lock nor a high-performance Wi-Fi lock; its discovery wait increases through 5, 10, and 20 seconds to a 30-second cap. While connected, both performance locks keep inference running with the screen off, so charging and thermal monitoring are recommended. If the device goes off-road, external AI is disabled, Wi-Fi disappears, or TCP disconnects, the locks are released immediately and automatic mode scans again if the address changed. For zero background use, press **중지** in the app; this disables automatic connection and ends discovery, inference, networking, and locks. The notification action also ends the current service but preserves the automatic-connection preference. Android or an OEM may defer discovery and reconnection while the phone sleeps.

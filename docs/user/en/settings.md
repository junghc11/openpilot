...
The Kotlin app for real-phone testing is under `tools/android_ai_client/`. It requires a 64-bit ARM phone with Android 10 or newer, and no YOLO ONNX model is bundled. For the first test, use YOLO11n Detection as a 640-input FP32 ONNX model without embedded NMS or dynamic input; segmentation, pose, classification, OBB, and end-to-end outputs are not supported. Select it in the Android app with **YOLO ONNX 모델 선택 (권장: YOLO11n 640)**, not on the C3X. The source has no device-model gate, but C3X is the current validation target; C3 and C4 still require an on-device road-stream and UI test. See the [English Android installation and pairing guide](../../../tools/android_ai_client/README.en.md) for the exact export command and APK installation.

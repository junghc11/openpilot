...
The Kotlin app for real-phone testing is under `tools/android_ai_client/`. It lets you select the C3X address and ports, a COCO YOLO ONNX model, confidence threshold, and target inference FPS. The app registers Android NNAPI first so supported operations can run on an NPU, DSP, or GPU, while unsupported operations remain on the ONNX Runtime CPU provider. If accelerated session creation fails, it automatically recreates the whole session on CPU and displays the selected backend in the status panel. A direct Qualcomm QNN backend is not bundled yet. Android 10 or newer is required, and a persistent notification remains visible while it continues with the screen off. Follow that directory's `README.md` for build, installation, and supported model formats.

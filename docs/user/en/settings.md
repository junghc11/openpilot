...
With `ExternalAIShowOverlay` enabled, supported classes appear as class-specific 3D shapes on the external Cluster HUD. An external monitor is not required: the C3X camera view shows class-specific 2D silhouettes, object names following the current `LanguageSetting`, and confidence. Both the C3X view and external Cluster HUD provide object labels for English, German, French, Portuguese, Spanish, Turkish, Ukrainian, Thai, Traditional Chinese, Simplified Chinese, Korean, and Japanese; changing the UI language updates them from the same translation catalogs. A compact device status panel also shows the phone connection, inference backend, round-trip latency, and current object count. The device shows a green `eNPU` badge when the phone selects an NNAPI or QNN accelerated backend, or a blue `eCPU` badge for ONNX Runtime CPU or CPU fallback. Both badges remain hidden while disconnected or invalid, and the active compute badge remains visible when object overlays are disabled. Because NNAPI may distribute operations across an NPU, DSP, and GPU, `eNPU` means that the external accelerated path was selected, not that every operation was measured on a physical NPU. Traffic lights and stop signs currently use 2D symbols on the device only. Distance derived from bounding-box height is approximate until camera calibration is completed; when a detection matches a nearby front-radar object, only its class information is added to the radar position. On-device coordinates are also debug-grade approximations until the transmitted frame crop and calibration are finalized. `ExternalAIDebug` remains inactive until a later stage. Validate with static or recorded imagery and a mock sender before enabling it in a vehicle.
...
6. Press **권장 모델 다운로드** or select another compatible ONNX model. Once a verified model is ready and automatic connection remains enabled, discovery starts without another button press. If discovery fails, enter the current device address and use **입력 IP로 시작**. Confirm **연결됨** plus an `eNPU` or `eCPU` badge.

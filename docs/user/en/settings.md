...
The Kotlin app for real-phone testing is under `tools/android_ai_client/`. It lets you select the C3X address and ports, a COCO YOLO ONNX model, confidence threshold, and target inference FPS. The first version uses ONNX Runtime CPU only; QNN/NPU and GPU acceleration are deferred until accuracy, heat, and latency of the CPU path are measured on the actual phone. Android 10 or newer is required, and a persistent notification remains visible while it continues with the screen off. Follow that directory's `README.md` for build, installation, and supported model formats.

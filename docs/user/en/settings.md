...
In Carrot Web, use **Settings > Display > External AI** to toggle `External Phone AI` (`ExternalAIEnabled`) and `Show External AI Objects` (`ExternalAIShowOverlay`). The former defaults to off (`0`), while the latter defaults to on (`1`). `ExternalAIEnabled` enables the experimental path that sends road video to an external Android phone and receives its object detections. It is currently limited to visualization and communication diagnostics. Its data is not delivered to steering, acceleration, braking, radar, or the safety model, and it does not modify or online-train the C3/C3X/C4 `modeld` result. When enabled, `phoneaid` runs only while on-road and opens the TCP video port (default `7724`) and UDP result port (default `7725`). Camera conversion runs only while a phone is connected to the video port. A disconnect or two seconds without a result immediately hides stale objects while normal driving operation continues.

Leave `ExternalAIPhoneIP` empty to accept any IPv4 client and result sender, or set it to accept only that address. `ExternalAIFramePort` selects the TCP video port and `ExternalAIResultPort` selects the UDP result port. Road video is reduced to 640×360 JPEG; use `ExternalAIFrameFPS` (1–15, default 5) and `ExternalAIJpegQuality` (30–95, default 75) to tune rate and quality. When the network is slow, a pending old frame is replaced by the newest frame so latency cannot build into a queue. `ExternalAIMaxLatencyMs` sets the maximum round-trip age using the monotonic timestamp originally attached by the CarrotPilot device (default 300 ms). The connection is not encrypted, so use it only on a trusted, dedicated vehicle network and configure the phone IP.

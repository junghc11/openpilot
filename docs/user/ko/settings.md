...
`ExternalAIShowOverlay`를 켜면 지원 차종을 외부 Cluster HUD의 차종별 3D 형상으로 표시합니다. 외부 모니터가 없어도 C3X 본체 카메라 화면에서 차종별 2D 실루엣, 현재 `LanguageSetting`에 맞춘 객체명과 신뢰도를 확인할 수 있습니다. C3X 본체와 외부 Cluster HUD 모두 영어·독일어·프랑스어·포르투갈어·스페인어·튀르키예어·우크라이나어·태국어·중국어 번체·중국어 간체·한국어·일본어 객체명을 지원하며, 언어를 바꾸면 같은 번역 카탈로그를 따라 표시가 변경됩니다. 본체 상단에는 스마트폰 연결 상태, 추론 백엔드, 왕복 지연과 현재 객체 수도 표시합니다. 연결된 스마트폰이 NNAPI 또는 QNN 가속 백엔드를 선택하면 APN 표시와 같은 초록색 `eNPU`, ONNX Runtime CPU 또는 CPU 폴백이면 파란색 `eCPU` 배지가 켜집니다. 연결 대기·유효하지 않은 상태에서는 두 배지 모두 꺼지며 객체 표시를 꺼도 연산 상태 배지는 유지됩니다. NNAPI는 연산을 NPU·DSP·GPU에 분배할 수 있으므로 `eNPU`는 외부 가속 경로가 선택되었음을 뜻하며 모든 연산이 물리 NPU에서 실행되었다는 계측 증명은 아닙니다. 신호등과 정지표지는 본체 화면의 2D 기호로만 표시합니다. 바운딩박스 크기로 계산한 3D 거리는 카메라 보정 전의 근사값이며, 가까운 전방 레이더 객체와 일치하면 레이더 위치에 차종 정보만 합칩니다. 본체 화면 좌표도 전송 영상의 크롭·보정 작업 전까지 디버그용 근사 위치입니다. `ExternalAIDebug`는 후속 단계 전까지 비활성입니다. 실제 차량에서 활성화하기 전에 정적·녹화 영상과 더미 송신기로 먼저 확인하세요.
...
6. 앱에서 **권장 모델 다운로드**를 누르거나 다른 호환 ONNX를 선택합니다. 검증된 모델이 준비되고 기본 자동 연결이 켜져 있으면 같은 망 검색을 바로 시작합니다. 검색이 실패한 경우에만 현재 기기 주소를 입력하고 **입력 IP로 시작**을 사용합니다. **연결됨**과 `eNPU` 또는 `eCPU` 표시를 확인합니다.

앱은 NNAPI를 우선 등록하며 가속 세션 생성에 실패하면 CPU로 자동 폴백합니다. 검색 중에는 모델·NPU 세션을 열지 않고 Wake Lock과 고성능 Wi-Fi Lock도 사용하지 않으며 검색 대기를 5·10·20초에서 최대 30초까지 늘립니다. 연결 중에는 화면이 꺼져도 추론하도록 두 성능 Lock을 사용하므로 절전 모드가 아니라 성능 모드이며 충전과 발열 확인이 필요합니다. 기기가 주차 상태가 되거나 외부 AI를 끄거나 Wi-Fi가 끊기면 Lock을 즉시 해제하고 주소 변경 시 같은 망을 다시 검색합니다. 완전히 사용하지 않을 때는 앱의 **중지**를 눌러 자동 연결 설정과 검색·추론을 모두 종료하세요. 알림의 **중지**도 현재 서비스를 종료하지만 자동 연결 설정은 보존합니다. Android 제조사 정책에 따라 화면이 꺼진 동안 검색과 재연결이 지연될 수 있습니다.

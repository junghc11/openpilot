...
웹당근의 **설정 > 화면 표시 > 외부 AI**에서 `외부 스마트폰 AI 사용`(`ExternalAIEnabled`)과 `외부 AI 객체 표시`(`ExternalAIShowOverlay`)를 토글로 설정할 수 있습니다. 전자는 기본값이 꺼짐(`0`), 후자는 켜짐(`1`)입니다. `ExternalAIEnabled`는 외부 안드로이드 스마트폰이 처리할 도로 영상을 보내고 객체 탐지 결과를 받는 실험 기능이며, 현재 단계에서는 시각화와 통신 진단에만 사용합니다. 이 데이터는 조향, 가감속, 제동, 레이더 또는 안전 모델에 전달되지 않으며 C3/C3X/C4의 기본 `modeld` 인식 결과를 수정하거나 온라인 학습시키지 않습니다. 기능을 켠 경우에만 주행 중 `phoneaid`가 실행되어 TCP 영상 포트(기본 `7724`)와 UDP 결과 포트(기본 `7725`)를 엽니다. 스마트폰이 영상 포트에 연결된 동안에만 카메라 영상을 변환합니다. 연결이 끊기거나 결과가 2초 동안 없으면 오래된 객체를 즉시 숨기며 기존 주행 기능은 계속 동작합니다.

`ExternalAIPhoneIP`가 비어 있으면 어떤 IPv4 클라이언트와 결과 발신자도 받을 수 있고, IP를 지정하면 그 주소만 허용합니다. `ExternalAIFramePort`는 TCP 영상 포트, `ExternalAIResultPort`는 UDP 결과 포트입니다. 영상은 도로 카메라를 640×360 JPEG로 줄여 보내며 `ExternalAIFrameFPS`(1~15, 기본 5)와 `ExternalAIJpegQuality`(30~95, 기본 75)로 전송률과 화질을 조정합니다. 네트워크가 느릴 때는 대기 중인 이전 영상을 새 영상으로 교체하여 지연이 쌓이지 않게 합니다. `ExternalAIMaxLatencyMs`는 CarrotPilot 기기가 프레임에 붙인 단조시계 기준 최대 왕복 지연(기본 300ms)입니다. 이 연결은 암호화되지 않으므로 차량의 신뢰할 수 있는 전용 네트워크에서 사용하고 스마트폰 IP를 지정하세요.

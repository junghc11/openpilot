...
실제 스마트폰 시험용 Kotlin 앱은 `tools/android_ai_client/`에 있습니다. Android 10 이상의 64비트 ARM 스마트폰이 필요하며 YOLO ONNX 모델은 앱에 포함되지 않으므로 별도로 선택해야 합니다. 현재 소스에는 기기 종류 제한이 없지만 검증 기준은 C3X이며, C3와 C4는 같은 브랜치의 도로 영상 스트림과 UI가 동작하는지 실기 확인이 필요합니다. 빌드·APK 설치와 지원 모델 형식은 해당 폴더의 `README.md`를 따르세요.

같은 Wi-Fi에 접속하는 것만으로 자동 연결되지는 않습니다. 앱에는 자동 검색·부팅 자동 시작 기능이 없습니다.
...
3. `ExternalAIPhoneIP`에는 스마트폰 IP를 입력합니다. Android 앱의 **기기 IP**에는 반대로 C3/C3X/C4가 할당받은 IP를 입력합니다.
4. 양쪽 포트를 바꾸지 않았다면 TCP `7724`, UDP `7725`를 유지합니다.
5. 기기를 주행 상태로 전환합니다. `phoneaid`는 주행 중에만 실행되므로 주차 화면에서는 앱이 연결되지 않습니다.
6. 앱에서 호환 YOLO ONNX 모델을 선택하고 **시작**을 누른 뒤 **연결됨**과 `eNPU` 또는 `eCPU` 표시를 확인합니다.

앱은 NNAPI를 우선 등록하며 가속 세션 생성에 실패하면 CPU로 자동 폴백합니다. 연결 중에는 화면이 꺼져도 추론하도록 지속 알림, 부분 Wake Lock과 고성능 Wi-Fi Lock을 사용하므로 절전 모드가 아니라 성능 모드이며 충전과 발열 확인이 필요합니다. 기기가 주차 상태가 되거나 외부 AI를 끄거나 Wi-Fi가 끊기면 두 성능 Lock을 해제하고 1·2·4·8·16·30초 간격의 저전력 재연결 대기로 전환합니다. 완전히 사용하지 않을 때는 앱이나 알림에서 **중지**를 눌러 서비스와 재연결 루프를 종료하세요. 중지 상태에서는 추론·네트워크·Wake Lock이 없고 Android 기본 절전 정책을 따릅니다. 앱은 부팅 후 자동으로 다시 시작하지 않으며 Android 제조사 정책에 따라 화면이 꺼진 동안 재연결이 지연될 수 있습니다.

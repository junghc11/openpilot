...
웹당근의 **설정 > 화면 표시 > 외부 AI**에서 `외부 스마트폰 AI 사용`(`ExternalAIEnabled`), `외부 AI 영상 전송 방식`(`ExternalAITransport`), `외부 AI 객체 표시`(`ExternalAIShowOverlay`)를 설정할 수 있습니다. 사용 설정은 기본적으로 꺼짐(`0`), 영상 전송은 H.264(`1`), 객체 표시는 켜짐(`1`)입니다. `ExternalAIEnabled`는 외부 안드로이드 스마트폰이 처리할 도로 영상을 보내고 객체 탐지 결과를 받는 실험 기능이며, 현재 단계에서는 시각화와 통신 진단에만 사용합니다. 이 데이터는 조향, 가감속, 제동, 레이더 또는 안전 모델에 전달되지 않으며 C3/C3X/C4의 기본 `modeld` 인식 결과를 수정하거나 온라인 학습시키지 않습니다. 기능을 켠 경우에만 주행 중 `phoneaid`가 실행되어 TCP 영상 포트(기본 `7724`)와 UDP 결과 포트(기본 `7725`)를 엽니다. 연결이 끊기거나 결과가 2초 동안 없으면 오래된 객체를 즉시 숨기며 기존 주행 기능은 계속 동작합니다.

`ExternalAIPhoneIP`가 비어 있으면 어떤 IPv4 클라이언트와 결과 발신자도 받을 수 있고, IP를 지정하면 그 주소만 허용합니다. `ExternalAIFramePort`는 TCP 영상 포트, `ExternalAIResultPort`는 UDP 결과 포트입니다. 기본 H.264 모드는 C3/C3X/C4의 Qualcomm 하드웨어 인코더로 도로 영상을 854×480, 20 FPS, 약 750 kbps로 보내며 Android `MediaCodec` 하드웨어 디코더를 사용합니다. YouTube Live 저화질 도로 모드가 실행 중이면 같은 854×480 H.264 출력을 공유하여 중복 인코딩을 피합니다. 중·고화질 또는 광각 YouTube 모드는 해상도·카메라가 다르므로 External AI가 JPEG 호환 경로를 사용합니다. 인코더가 3초 동안 준비되지 않으면 연결을 끊지 않고 640×360 JPEG로 자동 전환하며, H.264 키프레임이 다시 도착하면 복귀합니다. H.264 네트워크 큐가 밀리면 불완전한 P프레임을 보내지 않고 다음 IDR 키프레임부터 다시 시작합니다. JPEG 호환 모드에서만 `ExternalAIFrameFPS`(1~15, 기본 5)와 `ExternalAIJpegQuality`(30~95, 기본 75)가 적용되며 대기 중인 이전 JPEG는 최신 영상으로 교체됩니다. 앱의 목표 추론률은 영상 수신률과 별도로 1~20 FPS에서 설정합니다. `ExternalAIMaxLatencyMs`는 CarrotPilot 기기가 프레임에 붙인 단조시계 기준 최대 왕복 지연(기본 300ms)입니다. 설정 변경은 현재 주행을 끝내고 다시 시작한 뒤 적용됩니다. 연결은 암호화되지 않으므로 차량의 신뢰할 수 있는 전용 네트워크에서 사용하고 스마트폰 IP를 지정하세요.

`ExternalAIShowOverlay`를 켜면 지원 차종을 외부 Cluster HUD의 차종별 3D 형상으로 표시합니다. 외부 모니터가 없어도 C3X 본체 카메라 화면에서 차종별 2D 실루엣, 현재 `LanguageSetting`에 맞춘 객체명과 신뢰도를 확인할 수 있습니다. C3X 본체와 외부 Cluster HUD 모두 영어·독일어·프랑스어·포르투갈어·스페인어·튀르키예어·우크라이나어·태국어·중국어 번체·중국어 간체·한국어·일본어 객체명을 지원하며, 언어를 바꾸면 같은 번역 카탈로그를 따라 표시가 변경됩니다. 본체 상단에는 스마트폰 연결 상태, 추론 백엔드, 선택 입력 크기, `총 지연/AI 처리시간`과 현재 객체 수도 표시합니다. 연결된 스마트폰이 NNAPI 또는 QNN 가속 백엔드를 선택하면 APN 표시와 같은 초록색 `eNPU`, ONNX Runtime CPU 또는 CPU 폴백이면 파란색 `eCPU` 배지가 켜집니다. 연결 대기·유효하지 않은 상태에서는 두 배지 모두 꺼지며 객체 표시를 꺼도 연산 상태 배지는 유지됩니다. `QNN`은 CPU 폴백을 금지한 전체 그래프 HTP 세션에서 1회 예열 추론까지 성공한 상태입니다. `NNAPI`는 연산을 NPU·DSP·GPU에 분배할 수 있으므로 이때의 `eNPU`는 외부 가속 경로를 요청한 상태이며 모든 연산이 물리 NPU에서 실행됐다는 계측 증명은 아닙니다. 신호등과 정지표지는 본체 화면의 2D 기호로만 표시합니다. 바운딩박스 크기로 계산한 3D 거리는 카메라 보정 전의 근사값이며, 가까운 전방 레이더 객체와 일치하면 레이더 위치에 차종 정보만 합칩니다. 본체 화면 좌표도 전송 영상의 크롭·보정 작업 전까지 디버그용 근사 위치입니다. `ExternalAIDebug`는 후속 단계 전까지 비활성입니다. 실제 차량에서 활성화하기 전에 정적·녹화 영상과 더미 송신기로 먼저 확인하세요.
...
실제 스마트폰 시험용 Kotlin 앱은 `tools/android_ai_client/`에 있습니다. Android 10 이상의 64비트 ARM 스마트폰이 필요하며 YOLO ONNX 모델은 APK에 포함되지 않습니다. 첫 시험은 동적 입력 `YOLO11n Detection` FP32 ONNX와 기본 320 실행을 권장합니다. 416은 균형, 640은 작은 객체 품질 비교용이며 정적 모델은 모델 자체 입력을 사용합니다. 모델 내 NMS, segmentation·pose·classification·OBB·end-to-end 출력은 사용하지 않습니다. Android 앱의 **권장 모델 다운로드**를 누르고 Ultralytics 라이선스 안내를 확인하면 공식 v8.4.0 `yolo11n.onnx`를 직접 받으며, 고정된 파일 크기·SHA-256·ONNX 입출력이 모두 맞아야 내부 저장소에 설치합니다. 인터넷이 없거나 다른 호환 모델을 시험할 때는 **다른 ONNX 파일 선택**을 사용합니다. 현재 소스에는 기기 종류 제한이 없지만 검증 기준은 C3X이며, C3와 C4는 같은 브랜치의 도로 영상 스트림과 UI가 동작하는지 실기 확인이 필요합니다. 모델 출처·라이선스·APK 설치는 [Android 한글 설치·연결 안내](../../../tools/android_ai_client/README.ko.md)를 따르세요.

앱은 스마트폰 핫스팟에서 C3X 주소가 바뀌어도 저장 주소를 먼저 확인한 뒤 같은 사설 IPv4 `/24`의 영상 포트 하나만 검색합니다. TCP 연결 뒤 첫 4바이트가 JPEG `CAI1` 또는 H.264 `CAI2`인 기기만 선택하고 주소를 저장합니다. 자동 연결 설정과 모델이 저장돼 있으면 앱을 열 때 검색 서비스가 시작되며 기기를 찾는 즉시 버튼 없이 YOLO를 시작합니다. 최대 두 개의 로컬 `/24`만 확인하고 인터넷이나 임의 포트 범위는 스캔하지 않습니다. 부팅 자동 시작은 하지 않으므로 스마트폰 재부팅이나 앱 강제 종료 뒤에는 앱을 다시 열어야 합니다.

1. 스마트폰과 C3/C3X/C4를 같은 신뢰할 수 있는 Wi-Fi 또는 스마트폰 핫스팟에 연결하고 AP·클라이언트 격리를 끕니다.
2. 웹당근에서 `ExternalAIEnabled=1`을 설정하고 객체를 보려면 `ExternalAIShowOverlay=1`도 설정합니다.
3. 스마트폰 핫스팟에서는 `ExternalAIPhoneIP`에 핫스팟 게이트웨이인 스마트폰 주소를 입력합니다. 시험 중 주소를 모르면 비울 수 있지만 신뢰할 수 있는 전용망에서만 사용합니다.
4. 양쪽 포트를 바꾸지 않았다면 TCP `7724`, UDP `7725`를 유지합니다.
5. 기기를 주행 상태로 전환합니다. `phoneaid`는 주행 중에만 실행되므로 주차 화면에서는 앱이 연결되지 않습니다.
6. 앱에서 **권장 모델 다운로드**를 누르거나 다른 호환 ONNX를 선택하고, 입력 크기는 먼저 320으로 둡니다. 검증된 모델이 준비되고 기본 자동 연결이 켜져 있으면 같은 망 검색을 바로 시작합니다. 검색이 실패한 경우에만 현재 기기 주소를 입력하고 **입력 IP로 시작**을 사용합니다. **연결됨**과 `eNPU` 또는 `eCPU` 표시를 확인합니다.

앱 v0.6.0 기본 빌드는 Qualcomm QNN/HTP 전체 그래프, NNAPI, CPU 순서로 세션을 선택합니다. 다운로드한 동적 모델은 QNN을 열 때 선택한 320·416·640 입력으로 고정하며, QNN에서 지원하지 않는 연산이 있거나 HTP를 사용할 수 없으면 이유를 상태에 표시하고 NNAPI로 자동 폴백합니다. NNAPI는 NCHW를 강제하지 않으며, 가속 세션 생성도 실패하면 CPU로 폴백합니다. 상태 화면은 최근 120개 표본의 전화 처리 평균·p95와 JPEG 디코딩·전처리·ORT·후처리 시간을 따로 보여줍니다. p95가 평균보다 계속 커지거나 열 상태가 `성능 제한`이면 320과 낮은 FPS를 유지해야 합니다. 검색 중에는 모델·NPU 세션을 열지 않고 Wake Lock과 고성능 Wi-Fi Lock도 사용하지 않으며 검색 대기를 5·10·20초에서 최대 30초까지 늘립니다. 연결 중에는 화면이 꺼져도 추론하도록 두 성능 Lock을 사용하므로 절전 모드가 아니라 성능 모드이며 충전과 발열 확인이 필요합니다. 기기가 주차 상태가 되거나 외부 AI를 끄거나 Wi-Fi가 끊기면 Lock을 즉시 해제하고 주소 변경 시 같은 망을 다시 검색합니다. 완전히 사용하지 않을 때는 앱의 **중지**를 눌러 자동 연결 설정과 검색·추론을 모두 종료하세요. 알림의 **중지**도 현재 서비스를 종료하지만 자동 연결 설정은 보존합니다. Android 제조사 정책에 따라 화면이 꺼진 동안 검색과 재연결이 지연될 수 있습니다.

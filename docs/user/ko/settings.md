...
`ExternalAIEnabled`는 외부 안드로이드 스마트폰이 처리할 도로 영상을 보내고 객체 탐지 결과를 받는 실험 기능입니다. 기본값은 꺼짐(`0`)이며, 현재 단계에서는 시각화와 통신 진단에만 사용합니다. 이 데이터는 조향, 가감속, 제동, 레이더 또는 안전 모델에 전달되지 않습니다. 기능을 켠 경우에만 주행 중 `phoneaid`가 실행되어 TCP 영상 포트(기본 `7724`)와 UDP 결과 포트(기본 `7725`)를 엽니다. 스마트폰이 영상 포트에 연결된 동안에만 카메라 영상을 변환합니다. 연결이 끊기거나 결과가 2초 동안 없으면 오래된 객체를 즉시 숨기며 기존 주행 기능은 계속 동작합니다.

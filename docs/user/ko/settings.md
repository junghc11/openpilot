...
앱은 NNAPI를 우선 등록하되 NCHW를 강제하지 않으며, 가속 세션 생성에 실패하면 CPU로 자동 폴백합니다. 상태 화면은 최근 120개 표본의 전화 처리 평균·p95와 JPEG 디코딩·전처리·ORT·후처리 시간을 따로 보여줍니다. p95가 평균보다 계속 커지거나 열 상태가 `성능 제한`이면 320과 낮은 FPS를 유지해야 합니다. 검색 중에는 모델·NPU 세션을 열지 않고 Wake Lock과 고성능 Wi-Fi Lock도 사용하지 않으며 검색 대기를 5·10·20초에서 최대 30초까지 늘립니다. 연결 중에는 화면이 꺼져도 추론하도록 두 성능 Lock을 사용하므로 절전 모드가 아니라 성능 모드이며 충전과 발열 확인이 필요합니다. 기기가 주차 상태가 되거나 외부 AI를 끄거나 Wi-Fi가 끊기면 Lock을 즉시 해제하고 주소 변경 시 같은 망을 다시 검색합니다. 완전히 사용하지 않을 때는 앱의 **중지**를 눌러 자동 연결 설정과 검색·추론을 모두 종료하세요. 알림의 **중지**도 현재 서비스를 종료하지만 자동 연결 설정은 보존합니다. Android 제조사 정책에 따라 화면이 꺼진 동안 검색과 재연결이 지연될 수 있습니다.

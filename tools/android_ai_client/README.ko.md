...
NMS를 모델 안에서 끝내고 `[1, N, 6]`을 반환하는 내보내기 형식은 아직 지원하지 않습니다. v0.7.0 기본 APK는 공식 ONNX Runtime QNN AAR과 Qualcomm QNN Runtime을 포함합니다. 먼저 HTP에서 전체 그래프를 열고, 한 연산이라도 CPU가 필요하면 `session.disable_cpu_ep_fallback=1`에 의해 QNN 세션을 거부한 뒤 NNAPI, 마지막으로 CPU 순서로 폴백합니다. NNAPI 세션은 FP16을 허용하고 느릴 수 있는 NCHW 강제 옵션과 NNAPI CPU는 사용하지 않습니다. NNAPI가 선택되면 지원되는 그래프가 NPU·DSP·GPU와 ORT CPU에 혼합 배치될 수 있습니다.
...
앱의 **권장 모델 다운로드**를 누르고 Ultralytics 모델 라이선스 안내를 확인하면 공식 `ultralytics/assets` v8.4.0 Release에서 `yolo11n.onnx`를 직접 받습니다. 크기 `10,930,182`바이트와 SHA-256 `634279b40c07c6391472c51ad45b81ebc48706a9a1fe72dd3396322acd0c053b`를 고정하고, FP32 동적 실행 호환 입력과 일반 COCO YOLO 출력까지 검증한 뒤 앱 내부 저장소에 설치합니다. 공식 파일의 메타데이터는 `[-1,3,-1,-1] → [-1,84,-1]`이며 실행할 때 선택한 320·416·640으로 확정됩니다. 임시 파일은 검증 실패·취소 시 삭제하며 다운로드가 성공해야 자동 연결에 사용합니다. 모델은 앱 삭제 시 함께 제거됩니다.

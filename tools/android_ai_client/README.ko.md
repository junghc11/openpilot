...
| 모델 | 용도 | COCO mAP50-95 | 다운로드 | 첫 시험값 |
|---|---|---:|---:|---|
| YOLO11n | 속도 우선 | 39.5 | 10.4MB | 320, 10~15FPS |
| YOLO11s | 균형형·플래그십 권장 | 47.0 | 36.3MB | 320/416, 5~10FPS |
| YOLO11m | 정확도 우선 | 51.5 | 76.9MB | 320/416, 3~5FPS |

기본 320은 성능 우선, 416은 균형, 640은 작은 객체 품질 우선 시험값입니다. 입력 픽셀 수는 320이 640의 1/4이므로 먼저 320에서 지속 성능과 발열을 확인하세요. 세 모델의 파일 입출력은 동적 FP32이고 QNN/NNAPI 백엔드에서 FP16 내부 연산을 허용합니다. 정적 모델은 앱 선택값과 관계없이 모델 자체 입력 크기를 사용합니다. YOLO11l/x, YOLOv8 또는 직접 학습한 호환 모델도 수동 선택할 수 있지만 성능·출력 형식은 별도 검증이 필요합니다. YOLO26 end-to-end, segmentation, pose, classification, OBB 모델은 현재 지원하지 않습니다.

앱의 **권장 모델 다운로드**는 선택 목록의 YOLO11n/s/m을 공식 `ultralytics/assets` v8.4.0 Release에서 직접 받습니다. YOLO11n은 `10,930,182`바이트와 SHA-256 `634279b40c07c6391472c51ad45b81ebc48706a9a1fe72dd3396322acd0c053b`, YOLO11s는 `38,051,729`바이트와 `21d6650c5097610c92c76ce5e4b717976059169eaea4962035b90c6a92c07a8f`, YOLO11m은 `80,673,621`바이트와 `8a37b5c53ff642831aa454156b548ec2cf2537827445385c3e1c1b276cb666a3`를 고정 검증합니다. 이어서 FP32 동적 실행 호환 입력과 일반 COCO YOLO 출력까지 확인한 뒤 앱 내부 저장소에 각각 설치합니다. 공식 파일의 메타데이터는 `[-1,3,-1,-1] → [-1,84,-1]`이며 실행할 때 선택한 320·416·640으로 확정됩니다. 임시 파일은 검증 실패·취소 시 삭제하며 다운로드가 성공해야 자동 연결에 사용합니다. 모델은 개별 삭제할 수 있고 앱 삭제 시 모두 제거됩니다.
...
5. Android 앱에서 YOLO11n/s/m 중 하나를 선택해 **권장 모델 다운로드**를 누르거나 **다른 ONNX 파일 선택**으로 호환 모델을 지정합니다. 권장 모델은 다운로드 뒤 무결성과 입출력을 자동 검증합니다.
...
앱의 **YOLO 입력 크기**는 기본 320이며 목표 추론률은 1~20 FPS에서 선택합니다. 먼저 320·5FPS로 15분 이상 실행한 뒤 필요할 때만 FPS 또는 입력을 416·640으로 올리세요. 중앙 HUD의 `VIDEO FPS`는 C3X 원본 프레임 타임스탬프로 계산한 영상률, `AI FPS`는 현재 모델의 실측 처리율, `추종률`은 AI FPS/VIDEO FPS, `SKIP/s`는 두 속도의 차이입니다. 따라서 스마트폰 처리 루프가 느려져도 추종률이 100%로 잘못 보이지 않습니다. 객체 콘솔에는 시각, 프레임 ID, 객체명, 신뢰도, 원본 영상 기준 box와 center 픽셀 좌표가 최근 40개 분석 단위로 표시됩니다. 객체명은 Android 시스템 언어가 한국어이면 한글과 COCO 원문을 함께, 그 외 언어이면 COCO 영문으로 표시합니다.

상태 화면은 최근 120개 처리 표본의 전화 처리 평균과 p95, 현재 H.264/JPEG 디코딩·전처리·ORT 런타임·후처리 평균, 배터리 온도와 Android 열 상태를 구분해 표시합니다. C3X 상태에는 선택 입력 크기와 `총 지연/AI 처리시간`이 표시됩니다. 앱의 초록색 `eNPU`는 CPU 폴백을 금지한 Qualcomm QNN/HTP 전체 그래프 세션에서 예열 추론까지 성공한 경우에만 점등합니다. NNAPI 성공은 실제 NPU를 확정할 수 없으므로 주황색 `eACCEL`, CPU 폴백은 보라색 `eCPU`로 표시합니다. `QNN 폴백` 문구가 보이면 뒤의 원인을 기록하고 선택된 NNAPI 또는 CPU 성능으로 판단하세요.
...
권장 모델은 `https://github.com/ultralytics/assets/releases/download/v8.4.0/`의 `yolo11n.onnx`, `yolo11s.onnx`, `yolo11m.onnx`에서 HTTPS로 직접 다운로드하며 앱에 고정한 파일 크기·SHA-256과 ONNX 형식이 모두 맞아야 설치됩니다. 이 무결성 검사는 모델 라이선스를 대체하지 않습니다.

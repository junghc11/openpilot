...
v0.15.0 adds stable object track IDs, debounced traffic-light colors, automatic day/night preprocessing, and a thermal/load-aware FPS governor to the bright **Status, Model, and Log** tabs. **Model** can benchmark every installed model against the same live frame and configure separate confidence thresholds for all eight supported classes. **Status** can save the next raw frame and detection JSON as a false-positive or missed-detection sample. **Log** uses an internally scrolling X1/X2/X3-height console and includes track IDs and signal colors. Manual IP, ports, and the SSH deployment-command copy action remain under **Connection management**.
